...
interface CitiesSelectorProps {
    ...
    countryId?: string;
    selectedItem?: { id: string; country_id: string; name_ru: string; name_en: string };
...
export const CitiesSelector = memo(function CitiesSelector(
    ...
    const options = useMemo(() => {
        ...
        return data.map(item => {
            return {
                id: item.id,
                text: locale === 'en' ? item.name_en : item.name_ru
            };
        });
    }, [data, i18n.language]);
    ...
    const selectedText = useMemo(() => {
        ...
        return locale === 'en' ? selectedItem?.name_en : selectedItem?.name_ru;
